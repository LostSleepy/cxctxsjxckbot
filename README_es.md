...
Teto usa **Groq AI (Llama 3.1 8B)** para responder naturalmente en español. Tiene memoria de mensajes recientes y adapta su personalidad dinámicamente.

| Trigger | Descripción |
|---|---|
| `@Teto <mensaje>` | Menciona al bot para iniciar una conversación |
| Menciones de **Sleepy** (admin) | Teto defiende automáticamente a @Sleepy cuando otros lo mencionan |
| Cooldown | 8s por usuario (bypasseado para el admin) |

**Características:**
- Consciente del contexto: recuerda los últimos 15 mensajes (hasta 2000 caracteres)
- Adaptación automática de personalidad según el tono de la conversación
- Prompt de sistema dinámico según el contexto del servidor
...
| `cx!avatar [@usuario]` | `av` | Muestra la foto de perfil de cualquier miembro en alta resolución |
...
| `cx!teto` | — | 🥖 |
| `cx!servidor` | `server`, `serverinfo`, `guild`, `guildinfo` | Info completa del servidor: miembros, canales, boosts, dueño |
| `cx!rol [@rol]` | `role`, `roleinfo`, `rolinfo` | Info detallada de un rol: color, miembros, posición |
...
| `cx!top` | `ranking`, `leaderboard` | Muestra el ranking de aura del servidor (top 10) |
...
| `cx!fraude [@usuario]` | `fraud`, `fraudscan` | Fraud scan brainrot — analiza la choppedness de cualquiera 🔍 |
...
| `GROQ_API_KEY` | — | API Key de Groq para el chat con IA |
