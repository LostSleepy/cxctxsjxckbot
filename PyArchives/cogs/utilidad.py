...
class Utilidad(commands.Cog):
    ...
    async def cita(self, ctx, m1_id: str, m2_id: str):
        ...
        # Solo tú tienes la llave de este comando
        ...
        for raw_id in [m1_id, m2_id]:
            # Limpiar la ID por si viene como mención <@!9798...>
            ...
            # Buscar al miembro en los servidores donde está el bot
        ...
        # Reporte de resultados al privado
        msg = []
    ...
    async def ship(self, ctx, usuario1: discord.Member, usuario2: discord.Member = None):
        ...
        # Si solo mencionas a uno, el otro eres tú
        if usuario2 is None:
            usuario2 = usuario1
            usuario1 = ctx.author
        ...
        # Frases dinámicas según el porcentaje
    ...
    async def server_info(self, ctx):
        ...
        # Fecha de creación con timestamp de Discord
    ...
    async def echo(self, ctx, canal: discord.TextChannel, *, mensaje: str):
        ...
        try:
            ...
            await ctx.message.add_reaction("✅") # Confirmación discreta
    ...
    async def help_command(self, ctx):
        ...
        embed = discord.Embed(
            ...
            description="Aquí tienes la lista de comandos disponibles divididos por categorías. \nEl prefijo es `cx!`",
            color=discord.Color.blue()
        )
        ...
        embed.add_field(
            name="🛠️ Utilidad",
            value=(
                "`cx!avatar [@user]` - Mira el avatar de alguien.\n"
                "`cx!userinfo [@user]` - Información de un miembro.\n"
                "`cx!8ball [pregunta]` - Consulta a la bola 8.\n"
                "`cx!hora` - Reloj mundial (Madrid, Japón, NY).\n"
                "`cx!aleatorio` - Elige un usuario al azar.\n"
                "`cx!hola` - El bot te saluda con un GIF."
            ),
            inline=False
        )
        ...
        embed.add_field(
            name="🛡️ Moderación",
            value=(
                "`cx!mlshr [N o all]` - Borra mensajes del chat.\n"
                "`cx!ruleta` - Silencia a alguien al azar (10 min).\n"
                "`cx!angelguard` - Quita el silencio a todos."
            ),
            inline=False
        )

        # Sección de Jujutsu Kaisen
        embed.add_field(
            name="💥 Jujutsu Kaisen",
            value=(
                "`cx!de [@user]` - Expansión de Dominio.\n"
                "`cx!bf [@user]` - Destello Negro (Black Flash)."
            ),
            inline=False
        )
